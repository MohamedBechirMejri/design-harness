...
export const DEFAULT_DH_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".dh"),
);
...
const MODE_ARGS = {
  ...
  "dev:server": ["run", "dev", "--filter=dh"],
  "dev:web": ["run", "dev", "--filter=@dh/web"],
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("DH_PORT_OFFSET"),
  devInstance: optionalStringConfig("DH_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid DH_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `DH_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric DH_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed DH_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly dhHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(dhHome);

    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      PORT: String(webPort),
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      DH_HOME: resolvedBaseDir,
      DH_PORT: String(serverPort),
    ...
    if (noBrowser !== undefined) {
      output.DH_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.DH_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.DH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.DH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.DH_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.DH_LOG_WS_EVENTS;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read DH_PORT_OFFSET/DH_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.DH_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.DH_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  dhHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Design Harness data (equivalent to DH_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("DH_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to DH_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("DH_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to DH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("DH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to DH_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("DH_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to DH_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("DH_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to DH_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("DH_PORT")),
  ),
